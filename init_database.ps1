Write-Host "=== Genesis 2025 Database Initializer ==="

# 1. Copy CSV file to container
Write-Host "1. Copying l2_clean.csv to timescaledb container..."
if (Test-Path "l2_clean.csv") {
    docker cp l2_clean.csv timescaledb:/l2_clean.csv
} elseif (Test-Path "backend/dataset/l2_clean.csv") {
    docker cp backend/dataset/l2_clean.csv timescaledb:/l2_clean.csv
} else {
    Write-Warning "l2_clean.csv not found in root or backend/dataset. Skipping import."
    exit 1
}

# 2. Run SQL scripts inside container
Write-Host "2. Initializing schema, hypertable, and importing data..."
$sql = @"
...
$sql | docker exec -i timescaledb psql -U postgres -d trading_hub

Write-Host "✅ Database initialized and optimized successfully!" -ForegroundColor Green
